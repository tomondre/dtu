...
for s in routers:
    if level[s] == 2:  # L3
        C_a = cluster[s]
        for d in destinations:
            # Determine C_b (Cluster serving destination d)
            C_b = None
            ...
            if C_a != C_b and C_b != 'other':
                # Allowed receivers: L2 routers within the same cluster C_a
                allowed_receivers = [r for r in routers if level[r] == 1 and cluster[r] == C_a]
                if allowed_receivers:
                    # Router s must send destination d to at least one allowed receiver
                    solver.add(Or([send[(s, d, r)] for r in allowed_receivers]))
                else:
                    # If no allowed receivers, the constraint cannot be satisfied
                    solver.add(False)
    ...
    if level[s] == 1:  # L2
        ...
        for d in destinations:
            ...
            if d == 'dOther':
                C_b = 'other'
            if C_a != C_b and C_b != 'other':
                # Router s must send destination d to r1
                solver.add(send[(s, d, 'r1')] == True)
                # Router s must NOT send destination d to any other router
                for r in routers:
                    if r != 'r1':
                        solver.add(send[(s, d, r)] == False)
